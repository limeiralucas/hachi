...
fn test_load_rom_from_reader_success() {
    ...
    for (i, &byte) in test_data.iter().enumerate() {
        assert_eq!(
            chip8.memory[initial_pc as usize + i],
            byte,
            "Byte at position {} should be 0x{:02X}",
            i,
            byte
        );
    }
...
fn test_load_rom_from_reader_memory_overflow() {
    ...
    assert_eq!(
        ...
        "Error should be FileTooLarge, got: {:?}",
        error.kind()
    );
    assert!(
        error.to_string().contains("ROM too large"),
        "Error message should contain 'ROM too large', got: {}",
        error
    );
}
